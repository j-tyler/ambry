...
/**
 * Test utility to capture ByteBuf slices created during operations.
 *
 * This wrapper intercepts all slice creation methods (slice, retainedSlice, readSlice, readRetainedSlice)
 * and records them for inspection. Useful for detecting ByteBuf leaks in error paths.
 *
 * Usage:
 * <pre>
 *   ByteBuf buffer = ...;
 *   ByteBufSliceCapture capture = new ByteBufSliceCapture(buffer);
 *
 *   // Use capture in place of buffer
 *   someOperation(capture);
 *
 *   // Inspect captured slices
 *   List&lt;ByteBuf&gt; slices = capture.getCapturedSlices();
 *   for (ByteBuf slice : slices) {
 *     System.out.println("Slice refCnt: " + slice.refCnt());
 *   }
 * </pre>
 */
public class ByteBufSliceCapture extends ByteBuf {
  private static final Logger logger = LoggerFactory.getLogger(ByteBufSliceCapture.class);
  ...
  public ByteBuf readSlice(int length) {
    ByteBuf slice = delegate.readSlice(length);
    capturedSlices.add(slice);
    logger.debug("Captured readSlice(length={}): refCnt={}", length, slice.refCnt());
    return slice;
  }

  @Override
  public ByteBuf readRetainedSlice(int length) {
    ByteBuf slice = delegate.readRetainedSlice(length);
    capturedSlices.add(slice);
    logger.debug("Captured readRetainedSlice(length={}): refCnt={}", length, slice.refCnt());
    return slice;
  }

  // All other methods delegate to underlying buffer
  ...
  @Override public ByteBufAllocator alloc() { return delegate.alloc(); }
  ...
  @Override public ByteBuf getBytes(int index, ByteBuffer dst) { return delegate.getBytes(index, dst); }
  ...
  @Override public int getBytes(int index, GatheringByteChannel out, int length) throws IOException { return delegate.getBytes(index, out, length); }
  @Override public int getBytes(int index, FileChannel out, long position, int length) throws IOException { return delegate.getBytes(index, out, position, length); }
  @Override public CharSequence getCharSequence(int index, int length, Charset charset) { return delegate.getCharSequence(index, length, charset); }
  ...
  @Override public ByteBuf setBytes(int index, ByteBuffer src) { return delegate.setBytes(index, src); }
  ...
  @Override public int setBytes(int index, ScatteringByteChannel in, int length) throws IOException { return delegate.setBytes(index, in, length); }
  @Override public int setBytes(int index, FileChannel in, long position, int length) throws IOException { return delegate.setBytes(index, in, position, length); }
  @Override public ByteBuf setZero(int index, int length) { return delegate.setZero(index, length); }
  @Override public int setCharSequence(int index, CharSequence sequence, Charset charset) { return delegate.setCharSequence(index, sequence, charset); }
  ...
  @Override public ByteBuf readBytes(ByteBuffer dst) { return delegate.readBytes(dst); }
  ...
  @Override public int readBytes(GatheringByteChannel out, int length) throws IOException { return delegate.readBytes(out, length); }
  @Override public CharSequence readCharSequence(int length, Charset charset) { return delegate.readCharSequence(length, charset); }
  @Override public int readBytes(FileChannel out, long position, int length) throws IOException { return delegate.readBytes(out, position, length); }
  ...
  @Override public ByteBuf writeBytes(ByteBuffer src) { return delegate.writeBytes(src); }
  @Override public int writeBytes(java.io.InputStream in, int length) throws IOException { return delegate.writeBytes(in, length); }
  @Override public int writeBytes(ScatteringByteChannel in, int length) throws IOException { return delegate.writeBytes(in, length); }
  @Override public int writeBytes(FileChannel in, long position, int length) throws IOException { return delegate.writeBytes(in, position, length); }
  @Override public ByteBuf writeZero(int length) { return delegate.writeZero(length); }
  @Override public int writeCharSequence(CharSequence sequence, Charset charset) { return delegate.writeCharSequence(sequence, charset); }
  ...
  @Override public int forEachByte(ByteProcessor processor) { return delegate.forEachByte(processor); }
  @Override public int forEachByte(int index, int length, ByteProcessor processor) { return delegate.forEachByte(index, length, processor); }
  @Override public int forEachByteDesc(ByteProcessor processor) { return delegate.forEachByteDesc(processor); }
  @Override public int forEachByteDesc(int index, int length, ByteProcessor processor) { return delegate.forEachByteDesc(index, length, processor); }
  ...
  @Override public ByteBuffer nioBuffer() { return delegate.nioBuffer(); }
  @Override public ByteBuffer nioBuffer(int index, int length) { return delegate.nioBuffer(index, length); }
  @Override public ByteBuffer internalNioBuffer(int index, int length) { return delegate.internalNioBuffer(index, length); }
  @Override public ByteBuffer[] nioBuffers() { return delegate.nioBuffers(); }
  @Override public ByteBuffer[] nioBuffers(int index, int length) { return delegate.nioBuffers(index, length); }
  ...
  @Override public String toString(Charset charset) { return delegate.toString(charset); }
  @Override public String toString(int index, int length, Charset charset) { return delegate.toString(index, length, charset); }
