# Constructor Tracking and Custom Object Handlers for Wrapped ByteBuf Objects

## Overview

Tracking ByteBuf flows through wrapper objects requires **two complementary mechanisms**:

1. **`trackConstructors`** - Agent configuration to track constructor calls
2. **`ObjectTrackerHandler`** - Custom handler to extract ByteBuf from wrappers

Both are necessary because:
- `trackConstructors` tracks when ByteBuf enters a wrapper constructor
- `ObjectTrackerHandler` tracks when wrapper objects are passed to methods and extracts their ByteBuf for leak detection

This document covers both mechanisms and how they work together in Ambry.
...
## Part 2: ObjectTrackerHandler for Method Parameter Tracking

### Why trackConstructors Alone Isn't Enough

Consider this code flow:

```java
// Constructor tracked ✓
PutRequest request = new PutRequest(correlationId, clientId, blobId, properties,
                                     metadata, byteBuf, size, type, key);

// But what happens here? ❌
processRequest(request);  // request is passed, not byteBuf directly!

void processRequest(PutRequest request) {
    // How does tracker know request contains a ByteBuf?
    ByteBuf buf = request.getBlob();  // Need to extract ByteBuf!
    buf.writeBytes(...);
}
```

**Problem:** When wrapper objects are passed to methods (not ByteBuf directly), the tracker needs to:
1. Recognize the wrapper object contains a ByteBuf
2. Extract the ByteBuf from the wrapper
3. Track the ByteBuf's refCnt

This is where **`ObjectTrackerHandler`** comes in.

### AmbryByteBufObjectHandler Implementation

Located at: `ambry-test-utils/src/main/java/com/github/ambry/testutils/AmbryByteBufObjectHandler.java`
...
public class AmbryByteBufObjectHandler implements ObjectTrackerHandler {
  ...
  public boolean shouldTrack(Object obj) {
    if (obj instanceof ByteBuf) return true;  // Track raw ByteBuf

    String className = obj.getClass().getName();

    // Track all protocol messages
    if (className.startsWith("com.github.ambry.protocol.")) return true;
...
```
1. Constructor call:
   new PutRequest(byteBuf)
   └─> trackConstructors tracks this ✓

2. Method parameter:
   processRequest(request)
   └─> ObjectTrackerHandler.shouldTrack(request) → true
   └─> ObjectTrackerHandler.extractByteBuf(request) → byteBuf
   └─> ObjectTrackerHandler.getMetric(byteBuf) → refCnt
   └─> Flow continues! ✓
```

### Registration

The handler is automatically registered in `ByteBufTrackerListener.testRunStarted()`:

```java
@Override
public void testRunStarted(Description description) {
  AmbryByteBufObjectHandler handler = new AmbryByteBufObjectHandler();
  ObjectTrackerRegistry.setHandler(handler);
  System.out.println("[ByteBufTrackerListener] Registered AmbryByteBufObjectHandler");
}
```

This happens **before any tests run**, ensuring wrapper tracking works for all tests.

### Extraction Strategies

The handler uses multiple strategies to extract ByteBuf from wrappers:

1. **Public Getter Methods** (preferred, fastest):
   - `PutRequest.getBlob()` → ByteBuf
   - `BlobData.content()` → ByteBuf

2. **Reflection on Private Fields** (fallback):
   - `EncryptJob.blobContentToEncrypt` (private field)
   - `DecryptJob.encryptedBlobContent` (private field)
   - `NettyResponseChannel.Chunk.buffer` (private field)

3. **Common Field Name Probing** (for protocol classes):
   - Try: `blob`, `content`, `buffer`, `compositeSendContent`
